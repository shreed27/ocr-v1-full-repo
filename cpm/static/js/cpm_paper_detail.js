...
;(function($, undef) {
	...
	console.log('selpapername:' , selpapername);
        var curpaperid = $("#id_paperid").val();
        ...
        if (curpaperid != -1) {
            $.ajax({
                type: "GET",
                url: "/cpm/paper/updatename/",
                dataType: "json",
                data: {"paperid":curpaperid,
                       "papername":selpapername},
                success: function(data) {
			console.log('ok....im here to updatename');
			...
			$('#id_passpoint').val(data['passpoint']);
