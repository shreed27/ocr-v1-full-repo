...
from cpm.models import CPM_Itempool, CPM_Paper, CPM_Question, CPM_Assignment
...
class CPM_CustomPaperForm(forms.Form):

    def __init__(self, *args, **kwargs):
        owner = kwargs.pop('owner')
        super(CPM_CustomPaperForm, self).__init__(*args, **kwargs)
        if owner:
            studentpaper = CPM_Paper.objects.filter(owner=owner)
    ...
    def clean_questionlist(self):
        ...
        for q in qids:
            try:
                question = CPM_Question.objects.get(id=int(q))
            except:
                continue
            questions.append(question)
        return questions

class CPM_DetailSearchForm(forms.Form):
	def __init__(self,*args,**kwargs):
		pids = kwargs.pop('paper')
		stuids = kwargs.pop('student')
		super(CPM_DetailSearchForm,self).__init__(*args,**kwargs)
		try:
			papers = CPM_Paper.objects.filter(id__in = pids)
			ss = [(-1,'----')] + list(set([(p.subject.id, p.subject.subjectname) for p in papers]))
			papers = [(-1,'----')] +  [(p.id, p.ppr_papername)\
				for p in CPM_Paper.objects.filter(id__in = pids)]
...
class CPM_PaperSearchForm(forms.Form):

	def __init__(self, *args, **kwargs):
		super(CPM_PaperSearchForm, self).__init__(*args, **kwargs)
...
class CPM_AssignmentDetailForm(forms.Form):
	def __init__(self,*args,**kwargs):
		teacherValue = kwargs.pop('teacher')
		logger.info('CPM_AssignmentDetailForm 1')
		super(CPM_AssignmentDetailForm,self).__init__(*args,**kwargs)
		logger.info('CPM_AssignmentDetailForm 2')
		assignments = CPM_Assignment.objects.filter(teacher = teacherValue)
		logger.info('CPM_AssignmentDetailForm 3')
		es = [(-1,"New Assignment")]+[(e.id,e.asm_assignmentname) for e in assignments]
		papers = CPM_Paper.objects.filter(owner = teacherValue.user)
	...
	def clean_description(self):
		...
		print desc
		return desc
class CPM_ItemPoolDetailForm(forms.Form):
    def __init__(self,*args,**kwargs):
        teacher = kwargs.pop('teacher')
        super(CPM_ItemPoolDetailForm,self).__init__(*args,**kwargs)
        if teacher:
...
class CPM_PaperDetailForm(forms.Form):
	def __init__(self, *args, **kwargs):
		teacher = kwargs.pop('teacher')
		super(CPM_PaperDetailForm, self).__init__(*args, **kwargs)
		if teacher:
			papers = CPM_Paper.objects.filter(owner=teacher.user)
...
class CPM_FeedbackForm(forms.Form):
