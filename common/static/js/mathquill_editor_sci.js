...
/**
 * SciElement is the core Sci DOM tree node prototype.
 * Both SciBlock's and SciCommand's descend from it.
 */
function SciElement(){}
_ = SciElement.prototype;
_.prev = 0;
_.next = 0;
_.parent = 0;
_.firstChild = 0;
 ...
 * May be passed a SciFragment that's being replaced.
 */
function SciCommand(cmd, html_template, text_template, replacedFragment) {
...
_ = SciCommand.prototype = new SciElement;
_.initBlocks = function(replacedFragment) {
  ...
  if (self.html_template.length === 1) {
    ...
    self.jQ.data(jQueryDataKey).block =
      (replacedFragment && replacedFragment.blockify()) || new SciBlock;
...
function Symbol(cmd, html, text) {
  SciCommand.call(this, cmd, [ html ],
    [ text || (cmd && cmd.length > 1 ? cmd.slice(1) : cmd) ]);
}
_ = Symbol.prototype = new SciCommand;
_.initBlocks = $.noop;
...
/**
 * Children and parent of SciCommand's. Basically partitions all the
 * symbols and operators that descend (in the Sci DOM tree) from
 * ancestor operators.
 */
function SciBlock(){}
_ = SciBlock.prototype = new SciElement;
_.latex = function() {
...
/**
 * An entity outside the Sci DOM tree with one-way pointers (so it's only
 * a "view" of part of the tree, not an actual node/entity in the tree)
 * that delimit a list of symbols and operators.
 */
function SciFragment(parent, prev, next) {
  if (!arguments.length) return;

  var self = this;

  self.parent = parent;
  self.prev = prev || 0; //so you can do 'new SciFragment(block)' without
...
_ = SciFragment.prototype;
_.remove= SciCommand.prototype.remove;
...
/*********************************************
 * Root sci elements with event delegation.
 ********************************************/
...
function createRoot(jQ, root, textbox, editable, include_toolbar) {
  ...
  if (!textbox)
    jQ.addClass('sciquill-rendered-sci');

  root.jQ = jQ.data(jQueryDataKey, {
    block: root,
    revert: function() {
      jQ.empty().unbind('.sciquill')
        .removeClass('sciquill-rendered-sci sciquill-editable sciquill-textbox sciquill-editor mathquill-rendered-math mathquill-editable mathquill-textbox mathquill-editor')
        .append(contents);
    }
  });
  ...
  root.textarea = $('<span class="textarea"><textarea></textarea></span>')
    .prependTo(jQ.addClass('sciquill-editable'));
  var textarea = root.textarea.children();
  if (textbox)
    jQ.addClass('sciquill-textbox');
  if (include_toolbar)
    addToolbar(root, jQ);
  ...
  jQ.bind('focus.sciquill blur.sciquill', function(e) {
    textarea.trigger(e);
  }).bind('keydown.sciquill', function(e) { //see Wiki page "Keyboard Events"
    lastKeydn.evt = e;
  ...
  }).bind('keypress.sciquill', function(e) {
  ...
  }).bind('mousedown.sciquill', function(e) {
  ...
  }).bind('selectstart.sciquill', false).blur();
...
var test1 = '\\frac{d}{dx}\\sqrt{x}=\\frac[1][2]\\sum'
$.getJSON("/static/js/periodic_table_name_id.json",function(data){
  window.symbol_json=data.table
});
function addToolbar(root, jQ) {
  ...
  $.each(button_tabs, function(index, tab){
    tabs.push('<li><a href="#' + tab.name + '_tab_sci"><span class="sciquill-rendered-sci">' + tab.example + '</span>' + tab.name + '</a></li>');
    var buttons = [];
    $.each(tab.button_groups, function(index, group) {
      $.each(group, function(index, cmd) {
        var obj = new LatexCmds[cmd](undefined, cmd);
        buttons.push('<li><a class="sciquill-rendered-sci" title="' + (cmd.match(/^[a-zA-Z]+$/) ? '\\' + cmd : cmd) + '">' +
                     (html_template_overrides[cmd] ? html_template_overrides[cmd] : '<span style="line-height: 1.5em">' + obj.html_template.join('') + '</span>') +
                     '</a></li>');
      });
      buttons.push('<li class="sciquill-button-spacer"></li>');
    });
    panes.push('<div class="sciquill-tab-pane '+tab.name+'_tab_sci" id="' + tab.name + '_tab_sci"><ul>' + buttons.join('') + '</ul></div>');
  });
  
  // here modified toolbar will be add ///////////////////////////////////////////////////////
  toolbar_html= '<div class="sciquill-toolbar"><ul class="sciquill-tab-bar">' + tabs.join('');
  toolbar_html +='<li><a href="#chemistry_tab_sci"><span class="sciquill-rendered-sci">*</span>Chemistry</a></li>';
  toolbar_html += '</ul><div class="sciquill-toolbar-panes">' + panes.join('');
  toolbar_html +=''; // add search option
  toolbar_html +='<div class="sciquill-tab-pane chemistry_tab_sci" id="chemistry_tab_sci"><ul><li class="chemistry_text"><input type="text" id="placeSelect" class="placeSelect" /></li></ul></div>'
  toolbar_html += '</div></div><br>';
  root.toolbar = $(toolbar_html).prependTo(jQ);
  $('.placeSelect').select2({
    width: '200px',
    ...
    placeholder: "Search Periodic table",
    data:window.symbol_json//[{ 'id': 1, 'text': "Ford"}]
}).on("change",function(){
var symbolid=$(this).val();
if (symbolid){
var symbol_latex=symbolid.substring(symbolid.lastIndexOf(",")+1);
root.cursor.writeLatex(symbol_latex);
}
});
 // here it is finished //////////////////////////////////////////////////////////////////////////////
  //toolbar_html= '<div class="sciquill-toolbar"><ul class="sciquill-tab-bar">' + tabs.join('');
  //toolbar_html +='<li><a href="#chemistry_tab"><span class="sciquill-rendered-sci">*</span>Chemistry</a></li>';
  //toolbar_html += '</ul><div class="sciquill-toolbar-panes">' + panes.join('');
  //toolbar_html +=''; // add search option
  //toolbar_html +='<div class="sciquill-tab-pane chemistry_tab" id="chemistry_tab"><ul><li><select id="chemistry_select"><option value="Cu">Cu</option><option value="Cs">Cs</option><option value="Hg">Hg</option><option value="Pb">Pb</option></select></li><li class="chemistry_text"><input type="text" value="Cu"></li></ul></div>'
  //toolbar_html += '</div></div><br>';
  //root.toolbar = $(toolbar_html).prependTo(jQ);
  // BKP for tool bar ////////////////////////////////////////////////////////////////////////////////
 
 
  jQ.find('.sciquill-tab-bar li a').mouseover(function() {
    jQ.find('.sciquill-tab-bar li').removeClass('sciquill-tab-selected');
    jQ.find('.sciquill-tab-pane').removeClass('sciquill-tab-pane-selected');
    $(this).parent().addClass('sciquill-tab-selected');
    var select_id = $(this.href.replace(/.*#/, '#')).attr("id");
    $("."+select_id).attr('class',select_id+' sciquill-tab-pane sciquill-tab-pane-selected');
  });
  jQ.find('.sciquill-tab-bar li:first-child a').mouseover();
  jQ.find('a.sciquill-rendered-sci').mousedown(function(e) {
    e.stopPropagation();
  }).click(function(){
    //root.cursor.writeLatex(test1, true);/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    if ($.inArray(this.title, [test1]) !== -1){
...
function RootSciBlock(){}
_ = RootSciBlock.prototype = new SciBlock;
_.latex = function() {
  return SciBlock.prototype.latex.call(this).replace(/(\\[a-z]+) (?![a-z])/ig,'$1');
};
_.text = function() {
...
_.keydown = function(e)
{
  ...
  case 'U+0041':
    if (e.ctrlKey && !e.shiftKey && !e.altKey) {
      if (this !== this.cursor.root) //so not stopPropagation'd at RootSciCommand
        return this.parent.keydown(e);
...
function RootSciCommand(cursor) {
  SciCommand.call(this, '$');
...
_ = RootSciCommand.prototype = new SciCommand;
_.html_template = ['<span class="sciquill-rendered-sci"></span>'];
...
_ = RootTextBlock.prototype = new SciBlock;
_.renderLatex = function(latex) {
  ...
  for (var i = 0; i < latex.length; i += 1) {
    ...
    if (chunk[0] === '$') {
      ...
      var root = new RootSciCommand(cursor);
...
_.keydown = RootSciBlock.prototype.keydown;
_.textInput = function(ch) {
  if (this.skipTextInput) return;

  this.cursor.deleteSelection();
  if (ch === '$'){
    this.cursor.insertNew(new RootSciCommand(this.cursor));
    }
  else
...
function SupSub(cmd, html, text, replacedFragment) {
  SciCommand.call(this, cmd, [ html ], [ text ], replacedFragment);
}
_ = SupSub.prototype = new SciCommand;
_.latex = function() {
  ...
  if (this.respaced = this.prev instanceof SupSub && this.prev.cmd != this.cmd && !this.prev.respaced) {
    if (this.limit && this.cmd === '_') {
      this.jQ.css({
        ...
        marginRight: .1-Sci.min(this.jQ.outerWidth(), this.prev.jQ.outerWidth())/+this.jQ.css('fontSize').slice(0,-2)+'em' //1px adjustment very important!
      });
    }
    else {
      this.jQ.css({
...
function Fraction(replacedFragment) {
  SciCommand.call(this, '\\frac', undefined, undefined, replacedFragment);
  this.jQ.append('<span style="width:0">&nbsp;</span>');
}
_ = Fraction.prototype = new SciCommand;
_.html_template = [
...
// custom latex started ///////////////////////////////////////////////////////////

function custom1(replacedFragment) {
  SciCommand.call(this, test1, undefined, undefined, replacedFragment);
  this.jQ.append('<span style="width:0">&nbsp;</span>');
}
_ = custom1.prototype = new SciCommand;
_.html_template = [
    '<span class="selection">',
    '<big>∑</big><span>',
    '<span class="sqrt-prefix" style="font-size: 1.1454545454545455em;">√</span>',
    '<span class="sqrt-stem" style="border-top-width: 1.5px;"><var>x</var><span class="binary-operator">=</span></span>'
                              ];
_.text_template = ['(', '/', ')'];

LatexCmds[test1] = custom1

/////////////////////////////////////////////////////////////////////////////////////////////////
//LatexCmds.overline = LatexCmds.bar = bind(VanillaSymbol, '\\overline', '&part;');

function overline(replacedFragment) {
  SciCommand.call(this, "\\overline", undefined, undefined, replacedFragment);
  //this.jQ.append('<span style="width:0"></span>');
}
_ = overline.prototype = new SciCommand;
_.html_template = [
...
_.placeCursor = function(cursor) {
  if (this.firstChild.isEmpty()) {
    ...
    if (prev !== this.prev) {
      var newBlock = new SciFragment(this.parent, prev, this).blockify();
...
function SquareRoot(replacedFragment) {
  SciCommand.call(this, '\\sqrt', undefined, undefined, replacedFragment);
}
_ = SquareRoot.prototype = new SciCommand;
_.html_template = [
...
function Bracket(open, close, cmd, end, replacedFragment) {
  SciCommand.call(this, cmd,
    ['<span><span class="paren">'+open+'</span><span></span><span class="paren">'+close+'</span></span>'],
    [open, close],
    replacedFragment);
  this.end = end;
}
_ = Bracket.prototype = new SciCommand;
_.initBlocks = function(replacedFragment) {
...
function TextBlock(replacedText) {
  if (replacedText instanceof SciFragment)
    this.replacedText = replacedText.remove().jQ.text();
  else if (typeof replacedText === 'string')
    this.replacedText = replacedText;

  SciCommand.call(this, '\\text');
}
_ = TextBlock.prototype = new SciCommand;
...
_.textInput = function(ch) {
  ...
  if (ch !== '$')
    ...
    var next = new TextBlock(new SciFragment(this.firstChild, this.cursor.prev));
...
_ = InnerTextBlock.prototype = new SciBlock;
_.blur = function() {
  this.jQ.removeClass('hasCursor');
  if (this.isEmpty()) {
...
_.focus = function() {
  SciBlock.prototype.focus.call(this);
...
_ = LatexCommandInput.prototype = new SciCommand;
...
function Binomial(replacedFragment) {
  SciCommand.call(this, '\\binom', undefined, undefined, replacedFragment);
  this.jQ.wrapInner('<span class="array"></span>').prepend('<span class="paren">(</span>').append('<span class="paren">)</span>');
}
_ = Binomial.prototype = new SciCommand;
_.html_template =
...
function Vector(replacedFragment) {
  SciCommand.call(this, '\\vector', undefined, undefined, replacedFragment);
}
_ = Vector.prototype = new SciCommand;
...
LatexCmds.editable = proto(RootSciCommand, function() {
  SciCommand.call(this, '\\editable');
  ...
  this.firstChild.blur = function() {
    ...
    SciBlock.prototype.blur.call(this);
  };
...
LatexCmds.N = LatexCmds.naturals = LatexCmds.Naturals =
  bind(VanillaSymbol,'\\scibb{N}','&#8469;');

LatexCmds.P =
LatexCmds.primes = LatexCmds.Primes =
LatexCmds.projective = LatexCmds.Projective =
LatexCmds.probability = LatexCmds.Probability =
  bind(VanillaSymbol,'\\scibb{P}','&#8473;');

LatexCmds.Z = LatexCmds.integers = LatexCmds.Integers =
  bind(VanillaSymbol,'\\scibb{Z}','&#8484;');

LatexCmds.Q = LatexCmds.rationals = LatexCmds.Rationals =
  bind(VanillaSymbol,'\\scibb{Q}','&#8474;');

LatexCmds.R = LatexCmds.reals = LatexCmds.Reals =
  bind(VanillaSymbol,'\\scibb{R}','&#8477;');

LatexCmds.C =
LatexCmds.complex = LatexCmds.Complex =
LatexCmds.complexes = LatexCmds.Complexes =
LatexCmds.complexplane = LatexCmds.Complexplane = LatexCmds.ComplexPlane =
  bind(VanillaSymbol,'\\scibb{C}','&#8450;');

LatexCmds.H = LatexCmds.Hamiltonian = LatexCmds.quaternions = LatexCmds.Quaternions =
  bind(VanillaSymbol,'\\scibb{H}','&#8461;');
...
LatexCmds.heartsuit = bind(VanillaSymbol, '\\heartsuit', '&hearts;');
...
/* The main thing that manipulates the Sci DOM. Makes sure to manipulate the
HTML DOM to match. */

/* Sort of singletons, since there should only be one per editable sci
textbox, but any one HTML document can contain many such textboxes, so any one
JS environment could actually contain many instances. */

//A fake cursor in the fake textbox that the sci is rendered in.
...
_.seek = function(target, pageX, pageY) {
  ...
  //if no SciQuill data, try parent, if still no, forget it
  else {
    ...
    while (latex.length) {
      ...
      if (token.slice(0, 6) === '\\text{') {
        ...
        if (cursor.prev) //was a close-paren, so break recursion
          ...
          latex.unshift('{'); //in the ParenBlock in the sci DOM
      }
...
function Selection(parent, prev, next) {
  SciFragment.apply(this, arguments);
}
_ = Selection.prototype = new SciFragment;
_.jQinit= function(children) {
...
_.blockify = function() {
  ...
  return SciFragment.prototype.blockify.call(this);
};
_.detach = function() {
  var block = SciFragment.prototype.blockify.call(this);
...
$.fn.sciquill = function(cmd, latex) {
  switch (cmd) {
  case 'redraw':
  ...
  default:
    var textbox = cmd === 'textbox',
      ...
      RootBlock = textbox ? RootTextBlock : RootSciBlock;
    return this.each(function() {
...
//on document ready, sciquill-ify all `<tag class="sciquill-*">latex</tag>`
//elements according to their CSS class.
$(function() {
  $('.sciquill-editable').sciquill('editable');
  $('.sciquill-editor').sciquill('editor');
  $('.sciquill-textbox').sciquill('textbox');
  $('.sciquill-embedded-latex').sciquill();
});
