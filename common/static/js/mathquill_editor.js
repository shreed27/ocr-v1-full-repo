  ...
  root.jQ = jQ.data(jQueryDataKey, {
    ...
    revert: function() {
      jQ.empty().unbind('.mathquill')
        .removeClass('mathquill-rendered-math mathquill-editable mathquill-textbox mathquill-editor sciquill-rendered-sci sciquill-editable sciquill-textbox sciquill-editor')
        .append(contents);
    }
  });
...
function addToolbar(root, jQ) {
  ...
  var button_tabs = [
    ...
    { name: 'Delimiters',
      example: '{',
      button_groups: [["lfloor", "rfloor", "lceil", "rceil", "slash"]
      ]},
    { name: 'Misc',
      example: '&infin;',
      button_groups: [["forall", "ldots", "cdots", "vdots", "ddots", "surd", "triangle", "ell", "top", "flat", "natural", "sharp", "wp", "bot", "clubsuit", "diamondsuit", "caret", "underscore", "backslash", "vert", "perp", "nabla", "hbar", "AA", "circ", "bullet", "setminus", "neg", "dots", "Re", "Im", "partial", "infty", "aleph", "deg", "angle"]
      ]},
  ];
  ...
  $.each(button_tabs, function(index, tab){
    tabs.push('<li><a href="#' + tab.name + '_tab_math"><span class="mathquill-rendered-math">' + tab.example + '</span>' + tab.name + '</a></li>');
    var buttons = [];
    $.each(tab.button_groups, function(index, group) {
      $.each(group, function(index, cmd) {
        var obj = new LatexCmds[cmd](undefined, cmd);
        buttons.push('<li><a class="mathquill-rendered-math" title="' + (cmd.match(/^[a-zA-Z]+$/) ? '\\' + cmd : cmd) + '">' +
    ...
    panes.push('<div class="mathquill-tab-pane '+tab.name+'_tab_math" id="' + tab.name + '_tab_math"><ul>' + buttons.join('') + '</ul></div>');
  });
  root.toolbar = $('<div class="mathquill-toolbar"><ul class="mathquill-tab-bar">' + tabs.join('') + '</ul><div class="mathquill-toolbar-panes">' + panes.join('') + '</div></div><br>').prependTo(jQ);

  jQ.find('.mathquill-tab-bar li a').mouseover(function() {
    ...
    $("."+select_id).attr('class',select_id+' mathquill-tab-pane mathquill-tab-pane-selected');
  });
  jQ.find('.mathquill-tab-bar li:first-child a').mouseover();
...
function custom1(replacedFragment) {
  MathCommand.call(this, test1, undefined, undefined, replacedFragment);
  this.jQ.append('<span style="width:0">&nbsp;</span>');
}
_ = custom1.prototype = new MathCommand;
_.html_template = [
