...
class Answer:
    ...
    # Returns a hash from critique type to score (e.g., 'grammar' => 0.25)
    #
    def Critique(self, fulltext):
        self.text_critique = TextCritique()
        self.text_critique.analyze(fulltext)
        result = {'grammar-issues': self.text_critique.get_grammatical_errors(),
                  'percent-ungrammatical': self.text_critique.percent_grammar_errors()}
        debug_print("Answer.Critique() => %s" % result, 4)
        return result
...
if __name__ == '__main__':
    ...
    # Note: numbers in student text only used for Standard.Analysis, which is used for frequency analysis.
    ## OLD: stu_full_text = "0. Lawyers practice law.\n1 . The lawyer chased the vehicle downtown.\n2 . The vehicle chased back.\n"
    ## TEST: stu_full_text = "Lawyers practice law.\nThe lawyer chased the vehicle downtown.\nThe vehicle chased back.\n"
    ...
    if (len(sys.argv) > 1):
        ...
        if (sys.argv[1] == "--help"):
            print_stderr("Usage:\npython %s [key_text] [student_text] [marking_scheme]" % __file__)
            sys.exit()

        key_full_text = open(sys.argv[1], 'r').read()
    if (len(sys.argv) > 2):
        stu_full_text = open(sys.argv[2], 'r').read()
    if (len(sys.argv) > 3):
