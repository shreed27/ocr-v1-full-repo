...
Parameters for sentence matching (see answer.py):
    ...
    multisen_matchrate	0.30	percentage of sentences that must match
    multisen_threshold	0.40	similarity threshold when multiple sentences involved
...
For convenient testing of the new term expansion support, the following environment variables are supported:
    ...
    USE_PART_OF_SPEECH        False    Prefix terms with part-of-speech label (TODO: complete parenthetical)
...
For testing based on the linkage annotations, the following environment variables apply
    Variable                  Default  Description
    CHECK_LINKAGES	      False    Analyze sentence/clause correspondence annotations (e.g., [[s1c => k2a]]
    ALLOW_LOOSE_ANNOTATIONS   False    Allow for correspondence annotations without square brackets (provided on line by itself)
    OVERLAP_THRESHOLD	      0.5      Ratio of character offset overlap for lines to be considered a match
...
Similarly, there are a few other environment overrides:
    ...
    NUM_TRIALS		      10       Number of times to test each student repsonse for question 1
    MAX_ESSAYS		      32       Maximum number of essays to process (n.b., use only to speed up debugging)
    RANDOM_THRESHOLDS	      True     Whether Q1 test applies random sentence matching thresholds
    RANDOM_SEED		      0	       Random seed to use (if nonzero)

To enable a setting, set the corresponding environment variable to "True" or "1" (without quotation marks). For example, under the Bash Unix shell, the environment variables can be assigned by prefixing the command line with assignments as follows:
   VAR1=value1 VAR2=value2 command ...
For example,
   OVERLAP_THRESHOLD=0.25 DEBUG_LEVEL=5 NUM_TRIALS=1 CHECK_LINKAGES=1 SKIP_OVERRIDES=1 EXCLUDE_LONG_TESTS=1 PYTHONPATH="$PWD/mysite:$PATH" DJANGO_SETTINGS_MODULE=settings python algo/tests.py
