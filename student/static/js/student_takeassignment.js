...
;(function($, undef) {
    ...
    $(function(){
		...
		console.log('previous qids[i]', qids[i]);
        ...
        $("#next").click(function(){
		console.log('next qids[i]', qids[i]);
            submitanswer(qids[i]);
            if(i !== count){
                i += 1;
                $("#answer_editor").val("");
	...
	$("#saveanswer").click(function(){
		console.log('qids[i] save answer:' , qids[i]);
		saveanswer(qids[i]);
            	$("#question_process_show").text(count+1+"/"+qids.length);
	});
    ...
    ///============== Auto Saved/Manual Save=================================
	 
	var saveanswer = function(questionid){
		answer_html = $("#answer_editor").val();
		console.log('saving...' , answer_html);
		$.ajax({type: "POST",
		    url: "/student/answersave/",
		    dataType: "json",
		    data: {
		        "answer_html": answer_html, 
		        "csrfmiddlewaretoken": csrfvalue,
	...
	//var autosavetimer=setInterval(function () {
	//	console.log('qids[i]', qids[i]);
	////	saveanswer(qids[i]);
	//	}, 10000);

///============== End of Auto Saved/Manual Save=================================
    ...
    var loadquestion = function(questionid){
        if(questionid != -1){
			...
			console.log('loadquestion ... question/stuget');
...
//            thumbhtml += '<img src="/static/' + thumbnails[t][0] + '" id=' + thumbnails[t][3] +' alt="' + thumbnails[t][1].slice(0,5) + '" width="96" height="72"></img>';
