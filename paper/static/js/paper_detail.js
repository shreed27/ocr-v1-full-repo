...
;(function($, undef) {
    ...
    var setEditableText = function(is_new){
        // alert('is it new = '+is_new);
        var curpaperid = $("#id_paperid").val();
        var selpapername = $('#id_papername').val();
        if (is_new){
            
        }else{
            selpapername = $('#id_paperid').children('option:selected').text();
            $('#id_papername').val(selpapername);
            $(".editable").text($('#id_papername').val());
        }
        
        // alert('curpaperid = '+curpaperid);
        // alert('selpapername = '+$('#id_papername').val());
        if (curpaperid != -1) {
            // alert($('#id_papername').val());
            $.ajax({
                type: "GET",
            ...
            var new_paper=$('#id_papername').val();
            // alert("going to call new paper = "+new_paper);
            $.ajax({
                type: "GET",
                url: "/paper/updatename/",
                dataType: "json",
                data: {"paperid":curpaperid,
                       "papername":new_paper},
                success: function(data) {
                    // // alert('data.papername = '+data.papername)
                    // // alert('data.paperid = '+data.paperid)
                    $("#id_paperid").children('option:selected').text(data['papername']);
                    $("#id_paperid").children('option:selected').val(data['paperid']);
                    pullZtreeData();
                    // // alert("done")
