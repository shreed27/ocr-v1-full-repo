...
def normalize(text):
    ...
    ## OLD: text = text.replace("\xE2\x80\x98", "'")	# opening single quote (U+2018)
    ## text = text.replace("\xE2\x80\x99", "'")	# closing single quote (U+2019)
    ## text = text.replace("\xE2\x80\x9C", '"')	# opening double quote (U+201C)
    ## text = text.replace("\xE2\x80\x9D", '"')	# closing double quote (U+201D)
    ## text = text.replace("\xC2\xA0", " ")        # non-breaking space (U+00A0)
    ## text = text.replace("\xE2\x80\x93", "-")    # en dash (U+2013)
    ## take 2:
    ## text = text.replace(utf8(u'\u2018'), "'")	# opening single quote (utf8: "\xE2\x80\x98")
    ## text = text.replace(utf8(u'\u2019'), "'")	# closing single quote (utf8: "\xE2\x80\x99")
    ## text = text.replace(utf8(u'\u201C'), '"')	# opening double quote (utf8: "\xE2\x80\x9C")
    ## text = text.replace(utf8(u'\u201D'), '"')	# closing double quote (utf8: "\xE2\x80\x9D")
    ## text = text.replace(utf8(u'\u00A0'), " ")   # non-breaking space (utf8: "\xC2\xA0")
    ## text = text.replace(utf8(u'\u2013'), "-")   # en dash (utf8: "\xE2\x80\x93")
    ## take 3:
