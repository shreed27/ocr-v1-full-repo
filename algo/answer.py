...
class Answer:
    ...
    def __init__(self):
        ...
        self.only_match_word_tokens_once = get_property_value(settings, 'ONLY_MATCH_WORD_TOKENS_ONCE', False)
    ...
    # EX: ans.CalCosDist([{'StuS': 'The lawyer chased the vehicle.', 'No': 1, 'StuSVec': {'chased': 1.1736001944781467, 'lawyer': 2.3472003889562933, 'vehicle': 1.1736001944781467}, 'StuWords': ['lawyer', 'chased', 'vehicle']}, {'StuS': 'The vehicle chased back.', 'No': 2, 'StuSVec': {'chased': 1.1736001944781467, 'lawyer': 0, 'vehicle': 1.1736001944781467}, 'StuWords': ['vehicle', 'chased', 'back']}],
        # {'TotalS_No': [1], 'SenWords': ['attorney', 'chased', 'ambulance'], 'No': 1, 'KeyS': ' The attorney chased the ambulance.', 'Point_No': 'P1', 'KeyBVec': [], 'KeySVec': {'attorney': 2.3472003889562933, 'chased': 1.1736001944781467, 'ambulance': 1.1736001944781467}})
        # => (0.70710678118654757, {'StuS': 'The vehicle chased back.', 'No': 2, 'StuSVec': {'chased': 1.1736001944781467, 'lawyer': 0, 'vehicle': 1.1736001944781467}, 'StuWords': ['vehicle', 'chased', 'back']})
    ...
    def Mark(self, std_sentencelist, std_pointlist_no, ans_sentencelist):
        ...
        for point_no, std_sen in zip(std_pointlist_no, std_sentencelist):
            ...
            max_cos = 0
            #HACK SKB
            KeywordBVec = False if std_sen['KeyBVec'] else True
            ...
            # TODO SEN: check maxcos and save in list self obj
            # will be writed to DB on the Omitted part
            self.closeness_points.update({point_no: max_cos})
            ...
            self.detailedmarklist.append({'Point_No': point_no,
                                          ...
                                          # HACK SKB
                                          'KeywordBVecResult': KeywordBVec,
                                          'Sentence_Rate': sen_rate,
                                          'Only_eqn_no_text': only_eqn_no_text,
    ...
    def Comparison(self, marklist, rulelist):
        ...
        for rule in rulelist:
            ...
            if match:
                ##debug_print("Answer.Comparison() => %s" % str((rule['Mark'], rule['Point'])), level=5)
                return rule['Mark'], rule['Point']
            else:
                match = True
        ##debug_print("Answer.Comparison() => %s" % str((0, None)), level=5)
        return 0, None
    ...
    def Omitted(self, marklist, std_pointlist, rulelist):
        ...
        for point in std_pointlist:
            if 'P0.' not in point['Point_No']:
                text = latex_to_img(point["Point_Text"])
                if point['Point_No'] in marklist:
                    omitted.append('C' + point['Point_No'][1:] + " . " + text)
                    # update the closeness details
                    closeness_stats.update({point['Point_No']: True})
                    _corrct_pts.append(point['Point_No'])
                else:
                    omitted.append('W' + point['Point_No'][1:]  + " . " + text)
                    # update the closeness details
                    closeness_stats.update({point['Point_No']: False})
                    _wrong_pts.append(point['Point_No'])

        closeness_stats.update({'wrong_pts': _wrong_pts, 'correct_pts': _corrct_pts, 'closeness': self.closeness_points})
        ##debug_print("Answer.Omitted(_,_,_) => %s" % str((mark, omitted)), level=6)
        return mark, omitted, closeness_stats
    ...
    def Analysis(self, fulltext, textfdist, std_sentencelist, std_pointlist, rulelist):
        ...
        mark, omitted, closeness_stats = self.Omitted(marklist, std_pointlist, rulelist)
        ...
        return mark, marklist, omitted, closeness_stats
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) > 3):
        # Extract ampersand delimited point spec's, each with question number spec and point value (separated by comma)
        scheme = []
