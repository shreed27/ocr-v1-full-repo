...
if __name__ == '__main__' or __name__ == "django.core.management.commands.shell":
    ...
    if (len(sys.argv) > 12):
        ...
        if (not re.search(r"^\s*\d+\s", key_full_text)):
            print_stderr(
                "Warning: adding dummy question/point indictors to student text")
            key_full_text = "0 . " + key_full_text.replace("\n", " ") + "\n"
    if (len(sys.argv) > 14):
        # Extract ampersand delimited point spec's, each with question number
        # spec and point value (separated by comma)
        scheme = []
    ...
    # I don't know why they have put this one here. __author__='KS'
