...
if __debug__:
    ...
    use_logger = False                  # use python logging rather than stderr
    logger = None
    ## Uncomment following for debugging (e.g., defaults for when run under apache)
    ## use_logger = True; logger = logging; debug_level = 3
        ...
        if (debug_level >= level):
            if (output_timestamps):
                ...
                if output_timestamps_diff and (last_timestamp != None):
                    ...
                    timestamp_spec = "+" + re.sub(r"^0[0:]+", "", str(timestamp - last_timestamp))
                else:
...
# force_console_logging([name=__main__], [level=DEBUG]): ensures that console loggers activated
# Note: only intended for debugging purposes (e.g., modules invoked standalone)
#
def force_console_logging(name="__main__", level=logging.DEBUG):
    debug_print("force_console_logging(name=%s, level=%s" % (name, level), 4)
    logging.basicConfig()
...
def getenv_int (var, default=-1):
    return getenv_integer(var, default)
#
...
if __debug__:
    ...
    debug_level = int(env_debug_level) if env_debug_level else debug_level
