...
class Answer:
    ...
    def __init__(self):
        ...
        self.closeness = 0.0
        # Matching thresholds
        ## OLD: self.dist_threshold = 0.25
        self.dist_threshold = get_property_value(settings, 'DIST_THRESHOLD', 0.25)
        ...
        if __debug__: 
            self.dist_threshold = getenv_number("DIST_THRESHOLD", self.dist_threshold)
            ...
            self.only_match_word_tokens_once = getenv_boolean('ONLY_MATCH_WORD_TOKENS_ONCE', self.only_match_word_tokens_once)
    ...
    def CalCosDist(self, ans_sentencelist, std_sen):
        ...
        for stu_sen in ans_sentencelist:
            ...
            if self.only_match_word_tokens_once and (stu_freq_lookup_key not in stu_sen):
                # TODO: stu_sen[stu_freq_lookup_key] = stu_sen[stu_freq_master_key].copy
                stu_sen[stu_freq_lookup_key] = dict()
                for word in list(stu_sen[stu_freq_master_key].keys()):
    ...
    def EvaluateCloseness(self, std_sentencelist):
        ...
        debug_print("std_doc_keys: %s" % (list(std_document['KeySVec'].keys())), 6)
        ...
        debug_print("ans_doc_keys: %s" % (list(ans_document['StuSVec'].keys())), 6)
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) > 2):
        ...
        if (not re.search(r"^\s*\d+\s", key_full_text)):
            print_stderr("Warning: adding dummy question/point indictors to key text")
