...
class TestJinja2CompressorExtension(TestCase):
    ...
    def test_nonascii_js_tag(self):
        ...
        {% endcompress %}""")
        context = {'MEDIA_URL': settings.COMPRESS_URL}
        out = '<script type="text/javascript" src="/media/CACHE/js/e214fe629b28.js"></script>'
        self.assertEqual(out, template.render(context))
    ...
    def test_nonascii_latin1_js_tag(self):
        ...
        {% endcompress %}""")
        context = {'MEDIA_URL': settings.COMPRESS_URL}
        out = '<script type="text/javascript" src="/media/CACHE/js/be9e078b5ca7.js"></script>'
        self.assertEqual(out, template.render(context))
    ...
    def test_nonascii_inline_css(self):
        ...
        template = self.env.from_string('{% compress css %}'
                                        ...
                                        '/* русский текст */'
                                        '</style>{% endcompress %}')
        out = '<link rel="stylesheet" href="/media/CACHE/css/b2cec0f8cb24.css" type="text/css" />'
        settings.COMPRESS_ENABLED = org_COMPRESS_ENABLED
        context = {'MEDIA_URL': settings.COMPRESS_URL}
        self.assertEqual(out, template.render(context))
