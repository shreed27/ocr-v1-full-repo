...
class OfflineTestCaseMixin(object):
    ...
    def test_offline(self):
        ...
        self.assertEqual([
            '<script type="text/javascript" src="/media/CACHE/js/%s.js"></script>' % (self.expected_hash, ),
        ], result)
...
class OfflineGenerationBlockSuperTestCaseWithExtraContent(OfflineTestCaseMixin, TestCase):
    ...
    def test_offline(self):
        ...
        self.assertEqual([
            '<script type="text/javascript" src="/media/CACHE/js/ced14aec5856.js"></script>',
            '<script type="text/javascript" src="/media/CACHE/js/7c02d201f69d.js"></script>'
        ], result)
        rendered_template = self.template.render(Context(settings.COMPRESS_OFFLINE_CONTEXT))
        self.assertEqual(rendered_template, "".join(result) + "\n")
...
class OfflineGenerationTestCaseErrors(OfflineTestCaseMixin, TestCase):
    ...
    def test_offline(self):
        count, result = CompressCommand().compress(log=self.log, verbosity=self.verbosity)
        self.assertEqual(2, count)
        self.assertIn('<script type="text/javascript" src="/media/CACHE/js/3872c9ae3f42.js"></script>', result)
        self.assertIn('<script type="text/javascript" src="/media/CACHE/js/cd8870829421.js"></script>', result)
