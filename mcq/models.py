...
class MCQ_Paper(models.Model):
	...
	assignment = models.ForeignKey(MCQ_Assignment, verbose_name='Assignment', blank=True, null=True, on_delete=models.SET_NULL)
	def __unicode__(self):
...
class MCQ_Question(models.Model):
	...
	qtyn_type = models.CharField(max_length=20, verbose_name='Question Type',
		             blank=True, null=True)
	itempool = models.ForeignKey(MCQ_Itempool, verbose_name='Item Pool')
	paper = models.ManyToManyField(MCQ_Paper, verbose_name='Paper',
		                   blank=True, null=True)
	teacher = models.ForeignKey(TProfile, verbose_name='Teacher')
	qtn_questioncategory = models.ForeignKey(MCQ_QuestionCategory, verbose_name='Category',
		                blank=True, null=True)
...
class MCQ_Optionlist(models.Model):
	...
	opl_description = models.CharField(max_length=1000, verbose_name='Description',
		                   blank=True, null=True)
	opl_option = models.CharField(max_length=1000, verbose_name='Option',
		                   blank=True, null=True)
	opl_iscorrect = models.BooleanField(verbose_name='Is Correct')
	opl_question = models.ForeignKey(MCQ_Question, verbose_name='Question')
	...
	def __unicode__(self):
		return '[Name:%s,Is Corrrect:%s]' % (self.opl_name,  self.opl_iscorrect)
...
class MCQ_QuestionImage(models.Model):
	qti_imagename = models.CharField(max_length=30, verbose_name='Image Name')
	qti_description = models.CharField(max_length=100, verbose_name='Description', blank=True, null=True)
	qti_iscorrect = models.BooleanField(verbose_name='Is Correct')
	qti_abspath = models.CharField(max_length=50, verbose_name='Absulte Path')
	question = models.ForeignKey(MCQ_Question, verbose_name='Question')
	qti_digest = models.CharField(max_length=100, blank=True, null=True, verbose_name='md5')

	class Meta:
		verbose_name = 'Question Image'

	def __unicode__(self):
		return '[Image Name:%s, IsCorrect:%s, Abspath:%s]' % (self.qti_imagename, self.qti_iscorrect, self.qti_abspath)
class MCQ_StudentTakenTracking(models.Model):
	...
	def __unicode__(self):
		return 'Tracking Assingment taken' + str(self.id)
...
class MCQ_StudentRetakeAnswer(models.Model):
	...
	srta_taked = models.BooleanField(verbose_name='Question Taked')
	srta_stuansimages = models.ManyToManyField(MCQ_QuestionImage, verbose_name='stuansimages')

	def __unicode__(self):
		return 'StudentAnswerSheet_' + str(self.srta_question.qtn_name) + '_' + self.srta_student.user.username + '-----' + str(self.id)
...
class MCQ_StudentAnswer(models.Model):
	...
	stuansimages = models.ManyToManyField(MCQ_QuestionImage, verbose_name='stuansimages')
...
class MCQ_StudentAchocExam(models.Model):
	...
	sae_student_answer_images = models.ManyToManyField(MCQ_QuestionImage, verbose_name='stuansimages')
...
class MCQ_OptionCanvas(models.Model):
    ...
    def __unicode__(self):
        return 'Canvas id:%d name:%s optoin:%s  ' % (self.id, self.name, self.option)
...
class MCQ_OptionImage(models.Model):
	...
	qti_iscorrect = models.BooleanField(verbose_name='Is Correct')
	qti_abspath = models.CharField(max_length=50, verbose_name='Absulte Path')
	option = models.ForeignKey(MCQ_Optionlist, verbose_name='Option')
	qti_digest = models.CharField(max_length=100, blank=True, null=True, verbose_name='md5')
